...
6. When the `trythisapp` npm account requires TOTP, ask for a fresh authenticator code only after the dry run has passed, then run:
   `./.agents/skills/release-teamcopilot-npm/scripts/publish.sh --publish --skip-checks --otp <fresh-code>`
...
- Publish with a fresh TOTP code after a clean dry run:
  `Use $.agents/skills/release-teamcopilot-npm to publish this version from the trythisapp npm account with a fresh authenticator code, then create the matching GitHub release.`
- Publish a beta tag:
