...
function normalizeRelativePath(rawPath: string, allowEmpty: boolean): string {
    ...
    if (withForwardSlashes.startsWith("/")) {
        throw {
            status: 400,
            message: "path must be relative to the skill root"
        };
    }
    ...
    if (normalized === "." || normalized.startsWith("../") || normalized === "..") {
        throw {
            status: 400,
            message: "path escapes the skill root"
        };
    }
    return normalized;
}
...
function assertWorkflowRootNotSymlink(slug: string): void {
    const workflowRoot = getSkillPath(slug);
    ...
    if (rootLstat.isSymbolicLink()) {
        throw {
            status: 400,
            message: "Skill editor does not support symlinked workflow directories"
        };
    }
}

function assertRealPathWithinWorkflowRoot(slug: string, absolutePath: string): void {
    const realRoot = fs.realpathSync(getSkillPath(slug));
...
function assertNoSymlinkInAncestors(slug: string, absolutePath: string): void {
    const root = path.resolve(getSkillPath(slug));
    ...
    while (true) {
        ...
        if (lstat.isSymbolicLink()) {
            throw {
                status: 400,
                message: "Skill editor does not support symlinks"
            };
        }
        if (current === root) {
            return;
...
export function listSkillDirectory(slug: string, rawPath: string | undefined): WorkflowFileTreeResponse {
...
export function readSkillFileContent(slug: string, rawPath: string | undefined): WorkflowFileContentResponse {
...
export function saveSkillFileContent(slug: string, request: WorkflowFileSaveRequest): WorkflowFileSaveResponse {
...
export function createSkillFileOrFolder(slug: string, rawParentPath: string | undefined, name: string, kind: "file" | "directory"): WorkflowFileNode {
...
export function uploadSkillFileFromTempPath(slug: string, rawParentPath: string | undefined, name: string, tempFilePath: string): WorkflowFileNode {
    assertValidName(name);
    const parentRelativePath = normalizeRelativePath(rawParentPath ?? "", true);
    const parentAbsolutePath = resolveWorkflowTarget(slug, parentRelativePath);
    if (!fs.existsSync(parentAbsolutePath)) {
...
export function renameSkillPath(slug: string, rawPath: string | undefined, newName: string): { old_path: string; new_path: string; node: WorkflowFileNode } {
...
export function deleteSkillPath(slug: string, rawPath: string | undefined): void {
    const relativePath = normalizeRelativePath(rawPath ?? "", false);
    const absolutePath = resolveWorkflowTarget(slug, relativePath);
    if (!fs.existsSync(absolutePath)) {
        throw {
